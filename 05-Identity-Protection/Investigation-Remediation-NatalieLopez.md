...
Resolved. Natalie Lopez's risk state shows Remediated in the Risky users report.

Her account came up in a normal sign-in after CA06 had been enforced for several days. CA06 challenged the sign-in and required a secure password change, which she completed. That password change automatically remediated the underlying user risk, the risk state closed on its own the moment the control did its job, no manual dismissal from an admin was needed.

This is the outcome I was hoping to demonstrate by holding her risk state open instead of clearing it immediately after validation. The monitoring window gave the enforced policy an actual opportunity to catch a live sign-in and resolve the case itself, rather than the resolution being an admin decision made in isolation from any real enforcement.

---

## Closed

- Natalie Lopez: risk state Remediated
- CA06: confirmed working against live traffic, not just the original test trigger
- CA07: remains in Report-only, no further action needed for this case
